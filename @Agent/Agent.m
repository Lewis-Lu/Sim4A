...
%                 print = true;
        ...
        function [optimalVelocity, prefVelocity]= calOptVel(obj, neighbours, voMethods, CONFIG, phase)

            if ~isempty(neighbours.agent)
                for i = 1:size(neighbours.agent, 2) 
                    vo_tmp = obj.helperCalVelocityObstacle(neighbours.agent(i), voMethods(1));
                    if ~isreal(vo_tmp)
                        optimalVelocity = obj.helperCalCollisionVelocity(neighbours.agent(i));
                        return
                    end
                end
            end
            if ~isempty(neighbours.obstacle)
                for i = 1:size(neighbours.obstacle, 2) 
                    allVO = [allVO obj.helperCalVelocityObstacle(neighbours.obstacle(i), voMethods(2))];
                    if ~isreal(vo_tmp)
                        optimalVelocity = obj.helperCalCollisionVelocity(neighbours.agent(i));
                        return
                    end
            ...
            if ~isempty(neighbours.polygon)
                for i = 1:size(neighbours.polygon, 2) 
                    allVO = [allVO obj.helperCalVelocityObstacle(neighbours.polygon(i), voMethods(2))];
                    if ~isreal(vo_tmp)
                        optimalVelocity = obj.helperCalCollisionVelocity(neighbours.agent(i));
                        return
                    end
...
%             posRelativeDistance = norm(posRelativeVector) - obj.dist_epsilon;
    ...
    function collisionVelocity = helperCalCollisionVelocity(obj, that)
